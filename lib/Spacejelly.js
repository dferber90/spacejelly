...
var Spacejelly = function (args) {
	...
	var configPath = args.config || DEFAULT_SPACEJELLY_CFG_PATH;
	this.options = _.defaults(loadConfig.call(self, configPath), defaultConfig);
...
var loadConfig = function (configPath) {
	...
	if (!fs.existsSync(resolvedPath)) {
		log.warn('spacejelly: config file not found.')
		this.exit(EXIT_CODES.SPACEJELLY_FAILED);
	}
	...
	if (configPath.substr(-3).toLowerCase() === '.js') {
		...
		log.warn('spacejelly: unkown config file type:', configPath);
		this.exit(EXIT_CODES.SPACEJELLY_FAILED);
	}
};
