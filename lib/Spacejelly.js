...
_.extend(Spacejelly.prototype, {
	...
	readConfigs: function () {
		...
		log.info('spacejelly: reading configs');
	...
	setup: function () {
		...
		this.meteor.on('stop', function () {
			log.info('spacjelly: stopped meteor.');
		});
	...
	exit: function (result) {
		...
		if (this.meteor.exitCode != 0) {
			exitWithCode(EXIT_CODES.METEOR_FAILED);
		}

		if (this.meteor.mongo.exitCode != 0) {
			exitWithCode(EXIT_CODES.MONGO_FAILED);
		}

		if (this.selenium.exitCode != 0) {
			exitWithCode(EXIT_CODES.SELENIUM_FAILED);
		}

		/*
		if (this.nightwatch.exitCode != 0) {
			exitWithCode(EXIT_CODES.NIGHTWATCH_FAILED);
		}
		*/

		if (! result) {
			exitWithCode(EXIT_CODES.TESTS_FAILED);
		}
		
		log.info('spacejelly: ✓ tests successfull.');
		exitWithCode(EXIT_CODES.ALL_WELL);
	}
});

var exitWithCode = function (code) {
	log.debug('Spacejelly.exitWithCode', code);
	process.exit(code);
};
