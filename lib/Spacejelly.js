...
_.extend(Spacejelly.prototype, {
	...
	startServices: function () {
		...
		this.meteor.on('blocked', function (what) {
			if (what === 'port') {
				log.info('spacejelly: port is blocked. maybe meteor is running already?');
			} else if(what === 'mongo') {
				log.info('spacejelly: mongo failed to start. maybe meteor is running already?');
			} else {
				log.info('spacejelly: failed to start.');
			}
			self.exit(EXIT_CODES.METEOR_FAILED);
		});
